...
// 변수에 대한 타입표기
let myName: string = "Alice"; // 이는 선책사항이다.
...
function printName(obj: { first: string, middle?: string, last: string; }) {
...
function welcomePeople(param: string[] | string) {
...
function getFirstThree(param: string[] | string) {
...
function printCoordination(pt: Point) {
...
const useInterface = () => {
    ...
    const extendsInterface = () => {
        ...
        const bear: Bear = {name: "Thor", honey: true}
        bear.name;
        bear.honey;
    }
    ...
    const extendsType = () => {
        ...
        const bear: Bear = {name: "Tazan", honey: false}
    }
}
...
type key = typeof req[keyof typeof req];
...
//컴파일시 타겟 지정
// $ tsc --target es2020 2_everyday-types.ts